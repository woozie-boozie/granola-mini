...
- **AI meeting notes** — one click turns the transcript into a structured summary (TL;DR · key points · decisions · action items) with Claude
...
| Echo cancellation | Not yet — needs software AEC using the captured system audio as the reference (naive AVAudioEngine Voice Processing I/O does AEC but ducks the far-side output we capture; headphones sidestep the bleed today) |
| Cloud transcription | `src/main/deepgram.ts` (cloud streaming) — one socket per channel |
| Local transcription | `src/main/whisper.ts` — on-device via `whisper.cpp` (`whisper-server`, model resident on the GPU); swap live in the UI |
| Diarisation / attribution | mic = "You", system = "Them"; Deepgram `diarize` splits far-side speakers |
| Desktop UI | `src/renderer` — Electron + React live transcript, Cloud/Local toggle |
